  ...
  for (const token of tokens) {
    switch (token.type) {
      ...
      case 'html_inline': {
        ...
        // Convert <br> variants to newlines instead of appending raw HTML
        const converted = htmlContent.replace(/<br\s*\/?>/gi, '\n');
        appendText(state, converted);
        break;
      }
...
function mergeStyleSpans(spans: MarkdownStyleSpan[]): MarkdownStyleSpan[] {
  const sorted = [...spans].toSorted((a: MarkdownStyleSpan, b: MarkdownStyleSpan) => {
    if (a.start !== b.start) {
